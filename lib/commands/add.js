...
const add = async args => {
    if (args[0] == commands[2]) {
        ...
        if (Domain == '') console.log(`\n- ${color.redBright("error")}: no Domain name`); error = true
        if (Username == '' && Email == '') console.log(`- ${color.redBright("error")}: username, email field is empty`); error = true
        if (error == true) process.exit()
